...
class TinyRecursiveReasoningModel_ACTV1Config(BaseModel):
    ...
    no_ACT_continue: bool = (
        True  # No continue ACT loss, only use the sigmoid of the halt which makes much more sense
    )
...
class TinyRecursiveReasoningModel_ACTV1_Inner(nn.Module):
    ...
    def reset_carry(
        self, reset_flag: torch.Tensor, carry: TinyRecursiveReasoningModel_ACTV1InnerCarry
    ):
        return TinyRecursiveReasoningModel_ACTV1InnerCarry(
    ...
    def forward(
        ...
        # Optionally return internal states z_H, z_L after inner recursion (for RL)
        z_n = (z_H, z_L) if return_z else None

        return new_carry, output, (q_logits[..., 0], q_logits[..., 1]), z_n
...
class TinyRecursiveReasoningModel_ACTV1(nn.Module):
    ...
    def forward(
        ...
        # Forward inner model
        new_inner_carry, logits, (q_halt_logits, q_continue_logits), z_n = self.inner(
            new_inner_carry, new_current_data, return_z=return_z
        )
        ...
        # Add internal states to outputs if requested (for RL)
        if return_z and z_n is not None:
            outputs["z_n"] = z_n
        ...
        with torch.no_grad():
            ...
            if self.training and (self.config.halt_max_steps > 1):
                # Halt signal
                # NOTE: During evaluation, always use max steps, this is to
                # guarantee the same halting steps inside a batch for batching
                # purposes
                ...
                if not self.config.no_ACT_continue:
                    ...
                    _, _, (next_q_halt_logits, next_q_continue_logits), _ = self.inner(
                        new_inner_carry, new_current_data, return_z=False
                    )
