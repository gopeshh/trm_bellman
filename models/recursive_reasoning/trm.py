...
class TinyRecursiveReasoningModel_ACTV1Config(BaseModel):
    ...
    rl_disable_value_head_norm: bool = False  # If True, skip spectral_norm + Lv scaling on value head
...
class TinyRecursiveReasoningModel_ACTV1(nn.Module):
    ...
    def __init__(self, config_dict: dict):
        ...
        if self.config.rl_enable_contraction:
            ...
            # The 2x2 ablation showed value head norm (spectral_norm + Lv scaling) causes
            # training collapse, while z→z contraction alone is stable (Condition B).
            if self.value_head is not None:
                if self.config.rl_disable_value_head_norm:
                    # Skip value head normalization per config (Condition B: stable training)
                    print("[INFO] Value head norm: DISABLED by config (rl_disable_value_head_norm=True)")
                else:
                    # Apply full value head normalization (default)
