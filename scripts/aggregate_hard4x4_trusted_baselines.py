...
def _write_table_tex(output_dir: Path, summaries: dict[str, Any]) -> Path:
    ...
    upi_steps = _format_steps_k(summaries[UPI_METHOD]["train_steps"])
    ppo_steps = _format_steps_k(summaries["sb3_ppo"]["train_steps"])
    a2c_steps = _format_steps_k(summaries["sb3_a2c"]["train_steps"])
    lines = [
...
def _write_summary_md(
    ...
    upi_steps = upi["train_steps"]
    ppo_steps = ppo["train_steps"]
    a2c_steps = a2c["train_steps"]
    lines = [
        ...
        "|--------|-------|-------|--------------|--------|-------------|---------------------|",
        (
            f"| UPI-TRM | {upi_steps} | 10 | {_format_mean_std(upi['per_seed_success_rates'], 3)} | "
            f"[{upi['success_rate_ci_95'][0]:.3f}, {upi['success_rate_ci_95'][1]:.3f}] | -- | -- |"
        ),
        (
            f"| SB3 PPO | {ppo_steps} | 10 | {_format_mean_std(ppo['per_seed_success_rates'], 3)} | "
        ...
        (
            f"| SB3 A2C | {a2c_steps} | 10 | {_format_mean_std(a2c['per_seed_success_rates'], 3)} | "
