#!/bin/bash
# Run 2x2 contraction matrix on 6-8 empties dataset (20k steps)
# 4 configs × 3 seeds = 12 runs, 2 at a time on 2 GPUs

set -e

FBCODE_DIR="$HOME/fbsource/fbcode"
LOG_ROOT="/home/buiksat/trm_bellman/runs/feasibility_6to8empties_contraction_matrix_20k"
...
echo "Starting 6-8 empties contraction matrix (20k steps) at $TIMESTAMP"
...
for job in "${JOBS[@]}"; do
    ...
    # Wait for wave of 2 to complete
    if [ ${#PIDS[@]} -eq 2 ]; then
        echo "Waiting for wave of 2 jobs..."
        wait "${PIDS[@]}"
        echo "Wave complete!"
        PIDS=()
    fi
