...
Tasks
	...
	•	Objective: Implement c(x,y) ∈ [0, C_max] and shaped reward:
	•	From your draft:
r(s,a,s') = r0(s,a,s') + γ Φ(s') − Φ(s) with Φ(x,y)=c(x,y) and absorbing normalization.
	•	Checker score options (support both):
	1.	Constraint score: c = C_max - violations (no solution needed)
	2.	Exact score: if ground-truth solution exists, optionally track correctness (diagnostic only)
	•	Base reward r0:
	•	r0=+1 on transition that solves and enters absorbing
	•	else 0
	•	Absorbing-state convention: Implement the training-time equivalent of your V(s_abs)=-C_max shaping convention by:
	•	using terminal bootstrap value V_terminal = -C_max when done=True
	...
	•	Default action set: a = (i, j, d) with i,j∈[0..8], d∈[1..9] → 729 actions
	...
	•	Implement:
	•	done=True if solved OR if t == T_max
	•	default T_max sweepable (e.g., 81, 160, 256)
	•	Provide info with:
...
Checkpoint Criteria
	...
	•	shaped reward matches γ c_next − c_cur + terminal reward when solved
	•	✅ You can run python -m trm_rl.envs.sudoku to play a random agent for 10 episodes and print mean score.
	•	✅ T_max, shaped vs sparse reward, overwrite behavior are all configurable.
...
z^{t+1} = (Π_R ∘ f_θ)(z^t, y, x)
	•	Unrolled value: U_n(s) = V_ψ(z^{(n)}(s), x)
	•	K-step target: G^{(K)} = Σ_{k=0}^{K-1} γ^k r_k + γ^K \bar V(s_K)
	•	Value loss: L_val = E[(U_n(s0) − stopgrad(G^{(K)}))^2]
	•	Centered advantage (exact baseline):
Â(s,a) = Q̂(s,a) − E_{b~π(·|s)}[Q̂(s,b)]
	•	Mixture update: π_new = (1−α) π + α π0 and distill π_new → π_φ

UPI–TRM Pseudocode (Implementation-Oriented)

Initialize policy πφ, value (fθ, Vψ), target value Vψ̄, replay buffer B
for iteration = 1..N:
  # Collect data
  for env step:
    s=(x,y)
    z_n = Unroll(fθ, x, y, n, ΠR)
    a ~ πφ(.|x,y,z_n) with action-mask
    s', r, done = env.step(a)
  ...
  # Value update
  compute targets G^(K) using Vψ̄ and absorbing padding
  minimize (U_n(s0) - stopgrad(G^(K)))^2 w.r.t (θ, ψ)

  # Candidate policy update
  compute Q̂ and centered Â with exact baseline under πφ
  take gradient step to produce candidate policy π0

  # Conservative mixture + distillation
  π_new = (1-α)πφ + α π0   (distribution-level mixture)
  update πφ by minimizing KL(π_new || πφ) on batch states
...
def project_ball(z, R):  # Euclidean projection
	...
	•	Task 2.2: Implement policy head πφ for plan edits
...
class EditPolicyHead(nn.Module):
	...
	•	Task 2.3: Implement value head Vψ
...
B_V(s) = Σ_a π(a|s) Q̂(s,a) over valid actions.
	•	Compute Q̂(s,a):
	•	start with one-step:
Q̂(s,a) = r + γ V_target(s')
	•	then optionally implement K-step Q̂^(K) later.
	•	Signature:
...
def compute_centered_advantages(
	...
	•	Task 2.6: Implement candidate policy update π0
	•	Implement as:
...
def make_candidate_policy(policy: nn.Module, grads, step_size: float) -> nn.Module:
	...
	•	Task 2.7: Implement mixture policy π_new and distillation
	•	Mixture distribution:
	•	p_new = (1-α) p_old + α p_cand
	•	Distill by minimizing:
	•	KL(p_new || p_phi) over a minibatch of states
	•	Must support action masks.
	•	Task 2.8: Implement value training with K-step bootstrapped regression
	•	Target:
	•	G^(K) = Σ γ^k r_k + γ^K V_target(s_K) with absorbing padding
	•	Use target network for stability:
	•	ψ̄ ← τ ψ̄ + (1−τ) ψ
	•	Task 2.9: Add contraction diagnostics (Lz estimates)
	•	Log:
	•	Lz_hat via finite differences on replay batches
	•	Cz_hat = first-step latent delta norm
...
Checkpoint Criteria
	•	✅ Unit tests pass:
	•	exact centering: abs(sum_a pi(a|s)*A_hat(s,a)) < 1e-6
...
Your draft experiments compare:
	...
	4.	No-recursion baseline (direct (x,y)→(π,V))
...
Tasks
	...
	•	train/Lz_hat_p95
	•	train/centering_defect (should be ~0 for exact baseline)
...
Algorithms
	...
	•	No-recursion baseline: direct encoder → (π,V).
...
Hyperparameters
	•	Environment:
	•	γ=0.99
	•	T_max ∈ {81, 160} (use 160 for main)
	•	reward mode: shaped (main) + sparse (appendix)
	•	UPI–TRM core:
	•	n ∈ {5, 10} (default 10)
	•	K ∈ {3, 5} (default 5)
	•	α ∈ {0.01, 0.05} (default 0.05)
	•	contraction dial λ ∈ {0.8, 0.9} (controls effective Lz; default 0.9)
	•	target EMA: τ=0.995
	•	Optim:
...
Experiment 2: Four-Dial Ablation (n, λ, K, α)

Your draft sweep ranges:
	•	n ∈ {1,2,5,10,20}
	•	λ ∈ {0.7,0.8,0.9,0.95,1.0} (λ controls contraction / empirical Lz)
	•	K ∈ {1,3,5,10}
	•	α ∈ {0.01,0.05,0.1,0.2}
...
Experiment 3: Curriculum 4×4 → 9×9 vs Scratch
...
Hyperparameters
	...
	•	curriculum split: 20% (4×4) + 80% (9×9)
...
Your draft diagnostics:
	...
	2.	empirical Lz_hat over training
	3.	centering defect ε_cent
	4.	correlation between predicted penalty and observed instability

Implementation details
	•	TD residual proxy: empirical |U_n(s) − (r + γ U_n(s'))| or K-step version
	•	Lz_hat: finite difference on latent update map (sample pairs z,z')
	•	ε_cent: should be ~0 if exact baseline is used; log to verify
	•	penalty term proxy:
	•	estimate α * ε_A0 using batch estimate
ε_A0_hat = max_s |E_{a~π0} [Â − A_ref]|
(in practice, use |E_{a~π0} [Â]| as a proxy since true A_ref unknown)
...
6.1 Unrolling Bias Decay vs n (Fixed Policy π)
Goal: Show ||U_n − V^π_MC|| decreases with n; relate to Lz_hat.

Implementation
	•	Freeze policy π (trained or random).
	•	Estimate Monte Carlo V^π_MC with long rollouts (e.g., 512 steps or until termination).
	•	Compare U_n(s) for n∈{1,2,5,10,20}.
	•	Plot error vs n with multiple λ values.

Command

python -m trm_rl.train exp=theory_unrolling_decay agent=fixed_policy

6.2 Centering and α Experiment
Goal: Compare:
	•	(a) exact centering (sum over actions)
	•	(b) approximate baseline (sampled)
	•	(c) standard V-baseline (GAE style)

and show the α-dependent stability behavior.
...
Goal: Show correlation between:
	•	empirical residual ||U_n − T_K^π U_n|| (proxy)
...
Metrics
	...
	•	drift proxy C_drift_hat(n) = ||z_t^(n) − z_init(x,y_t)||
...
Tasks
	...
	•	diagnostics plots (Lz_hat, TD residual)
...
Minimal "ICML submission set" (finish by Jan 05)
	...
	2.	Dial ablations: n + α sweeps (2 seeds)
	3.	Two key diagnostics plots: Lz_hat and TD residual over time
...
1) NaNs / divergence in UPI–TRM
	...
	•	increase target EMA smoothing (τ closer to 1)

2) Centering defect not ~0
	•	Likely cause: baseline computed with sampling or mask mismatch.
	•	Fix: ensure baseline sums only over valid actions with properly normalized π.
...
4) Reward shaping behaves oddly
	•	Fix: verify r = γ*c_next − c_cur on nonterminal transitions (unit test).
	•	Ensure terminal handling matches your absorbing convention (bootstrap V_terminal=-C_max).
