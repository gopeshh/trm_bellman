"""
Tests for theory metrics computation (C_z, L_z, L_v, unrolling term, Bellman residual).
These correspond to the bounds in Section 4-5 of the paper.
"""
import torch
...
def test_estimate_Cz_returns_positive_value():
    ...
    assert isinstance(hat_Cz, float)
    assert hat_Cz >= 0.0
    assert hat_Cz < float("inf")


def test_estimate_local_Lz_returns_reasonable_value():
    ...
    assert isinstance(hat_Lz, float)
    assert hat_Lz >= 0.0
    # Without contraction, L_z can be >= 1, but should be finite
    assert hat_Lz < 100.0


def test_estimate_Lv_returns_positive_value():
    ...
    assert isinstance(hat_Lv, float)
    assert hat_Lv >= 0.0
    assert hat_Lv < float("inf")


def test_compute_unrolling_term_proxy():
...
def test_compute_empirical_bellman_residual():
    ...
    assert metrics["bellman_residual_mean"] >= 0.0
    assert metrics["bellman_residual_max"] >= metrics["bellman_residual_mean"]


def test_unrolling_term_decays_with_n():
    ...
    for i in range(len(terms) - 1):
        assert terms[i] > terms[i + 1], f"Expected {terms[i]} > {terms[i+1]}"
